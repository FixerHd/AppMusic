...
public final class TDSUsuarioDAO implements UsuarioDAO {
	...
	private Entidad usuarioToEntidad(Usuario usuario) {
		...
		eUsuario.setPropiedades(new ArrayList<Propiedad>(Arrays.asList(new Propiedad(NOMBRE, usuario.getNombre()),
				...
				new Propiedad(FECHA_NACIMIENTO, usuario.getFechaNacimiento()), new Propiedad(PREMIUM, usuario.getPremium()), new Propiedad(PLAYLISTS, usuario.getPlayliststoEntity()))));
		return eUsuario;
	}
	...
	public void update(Usuario usuario) {
		...
		for (Propiedad prop : eUsuario.getPropiedades()) {
			if (prop.getNombre().equals(PASSWORD)) {
				...
				prop.setValor((usuario.getPlayliststoEntity()));
			}
